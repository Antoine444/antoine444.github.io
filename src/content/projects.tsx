        ...
        claims: [
            {
                ...
                condition: 'only when the drift is hedging-relevant — volatility, joint, or large price — and blocks carry ≥ 5k paths',
                tone: 'positive',
            },
            {
                figure: 'worse',
                metric: 'than uniform weighting, across every drift type',
                condition: 'at 1k paths per block, for every drift type tested',
                tone: 'adverse',
            },
            {
                figure: 'flat',
                metric: 'mean PnL — flat to marginally worse in every regime',
                condition: 'in every regime — the gain is dispersion and tail risk, never return',
                tone: 'neutral',
            },
        ],
        body: () => (
    ...
    /* ------------------------------------------------------------------ */
    {
        ...
        claims: [
            ...
            },
            {
                ...
                condition: 'messages batched 8 per datagram, retransmitted until acknowledged',
                tone: 'neutral',
            },
            {
                figure: '50',
                metric: 'lattice agreement slots running in parallel',
                condition: 'run concurrently, so one slow slot does not stall the pipeline',
                tone: 'neutral',
            },
        ],
        body: () => (
        ...
        claims: [
            {
                ...
                condition: 'memory independent of region size; collisions cost false aborts, not correctness',
                tone: 'neutral',
            },
            {
                figure: '1 atomic',
                metric: 'load reads both the lock bit and the version',
                condition: 'version in the high bits, lock bit in the LSB of a single uint64_t',
                tone: 'neutral',
            },
            {
                figure: '0 rollback',
                metric: 'work on abort — writes are deferred to commit',
                condition: 'speculative allocations freed on abort, frees deferred to commit',
                tone: 'neutral',
            },
        ],
        body: () => (
    ...
    /* ------------------------------------------------------------------ */
    {
        ...
        claims: [
            ...
            },
            {
                ...
                condition: 'measured on the FAISS-indexed RAG layer, not on accuracy',
                tone: 'positive',
            },
            {
                figure: '−75%',
                metric: 'memory, retaining 96.2% of full-precision performance',
                condition: 'a memory saving, not an accuracy gain',
                tone: 'positive',
            },
        ],
        body: () => (
        ...
        claims: [
            {
                ...
                condition: 'the corpus the whole analysis runs over',
                tone: 'neutral',
            },
            {
                figure: '14GB+',
                metric: 'metadata corpus processed within an 8–32GB RAM budget',
                condition: 'chunked pipeline — the corpus is never held in memory whole',
                tone: 'neutral',
            },
            {
                figure: '8.6B+',
